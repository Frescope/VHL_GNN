...
def feat_extract_v2(frames, spectro, smile, weights, biases, audio_weights, audio_biases):
    spectrograms = tf.reshape(spectro, shape=(-1, A_HEIGHT, A_WIDTH))  # 6b*384*400
    EIfeat = tf.reshape(smile, shape=(-1, A_IS_LEN, 1))
    _, audio_out = AudioNet.audionet(spectrograms, EIfeat, audio_weights, audio_biases)  # 6b*8*8*128
...
def run(data):
    ...
    with tf.Graph().as_default():
        ...
        varlist_visual_load = list( set(varlist_visual) - set([weights['wc5b'],biases['bc5b']]) )
        varlist_audio_load = list( set(varlist_audio) - set([audio_weights['wc5'],audio_biases['bc5']]) )
        ...
        for gpu_index in range(GPU_NUM):
            with tf.device('/gpu:%d' % gpu_index):
                ...
                # predict scores
                audio_out, visual_out = feat_extract_v2(frames, spectro, smile, weights, biases, audio_weights, audio_biases)
        ...
        for i in range(len(vids)):
            ...
            # check 检查输出顺序
            features_audio_ovr = np.array(feats_audio_list).reshape((-1,A_NUM,8,8,128))  # ?*6b*8*8*128
            ...
            np.save(FEATURE_BASE+vid+'//features_visual_ovr.npy',features_visual_ovr[:len(data[vid]['frame'])])
            np.save(FEATURE_BASE+vid+'//features_audio_ovr.npy',features_audio_ovr[:len(data[vid]['frame'])])
